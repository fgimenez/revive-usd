...
function matchEthCall(body: { method: string; params?: unknown[] }, to: string, selector: string): boolean {
  ...
  return params[0].to?.toLowerCase() === to && (params[0].data ?? '').toLowerCase().startsWith(selector)
}
...
test('withdraw revert: shows error message', async ({ page }) => {
  ...
  // Make the mock wallet reject the next transaction
  await page.evaluate(() => {
    ;(window as Window & { __mockWalletShouldFail?: string }).__mockWalletShouldFail = 'execution reverted: InsufficientCollateralRatio()'
  })

  await expect(page.getByRole('button', { name: 'Withdraw' })).toBeVisible({ timeout: 10_000 })

  // Fill the collateral amount input (first number input = collateral section)
  await page.locator('input[type="number"]').first().fill('10')
  await page.getByRole('button', { name: 'Withdraw' }).click()

  // Error message should appear
  await expect(
    page.locator('div').filter({ hasText: /Transaction failed|reverted|InsufficientCollateralRatio/ }).last()
